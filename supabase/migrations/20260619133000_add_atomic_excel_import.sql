create or replace function public.import_catalog_products(
  ...
  input_catalog_slug text default 'catalogo-principal'
)
returns jsonb
language plpgsql
security definer
...
revoke all on function public.import_catalog_products(jsonb, text) from public;
grant execute on function public.import_catalog_products(jsonb, text) to authenticated;

create policy "admins read generated pdf files"
  on storage.objects for select
  to authenticated
  using (
    bucket_id = 'catalog-pdfs'
